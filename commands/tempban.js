...
module.exports = {
    run: async (message, args) => {
        ...
        message.channel.send(new Discord.MessageEmbed()
            .setAuthor(`${member.user.tag} a été banni pendant ${humanizeDuration(duration, {language: 'fr'})} pour ${reason} !`, member.user.displayAvatarURL()))
            ...
            message.channel.send(new Discord.MessageEmbed()
            .setColor(`#00cfff`)
            .setAuthor(`${member.user.tag} a été débanni.`, member.user.displayAvatarURL()))
        }, duration)
    },
    name: 'tempban',
    guildOnly: true,
